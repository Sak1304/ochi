...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] w-full px-10 py-8 flex backdrop-blur justify-between place-items-center transition-transform duration-300 ${showNavbar ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className='logo'>
        ...
        <div className='links flex gap-10'>
            {["Services", "Our Work", "About Us" ,"Insights","Contact us"].map((item,index) => (
                <a key={index} className={`text-lg capitalize cursor-pointer  font-light ${index === 4 && "ml-32"}`}>{item}</a>
            ))}
        </div>
    </div>
  )
